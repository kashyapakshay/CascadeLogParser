...
def parse(blob):
	...
	for line in blob.splitlines():
		if line[0].isdigit():
			...
			prevMap = _parseString(line)
		else:
			if prevMap is not None:
				prevMap['message'] += line.replace('\t', ' ').replace('"', '\\"') + "<br />"

if __name__ == '__main__':
	logFileName = 'Cascade.log'
	logContent = ''

	with open('cascade.log', 'r') as log: 
		blob = log.read()

	logContent = '{'
	logContent += '"logName": "' + logFileName + '", "logContent": ['

	for logMap in parse(blob):
		logContent += '''{
			"date": "''' + logMap["date"] + '''",
			"time": "''' + logMap["time"] + '''",
			"type": "''' + logMap["type"] + '''",
	...
	with open('cascade.json', 'w') as jsonLog: 
